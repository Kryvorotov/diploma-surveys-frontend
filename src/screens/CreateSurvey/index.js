...
function CreateSurvey() {
  ...
  const [startAt, setStartAt] = useState(
    survey ? convertToInput(survey.startAt) : new Date()
  );
  const [endAt, setEndAt] = useState(
    survey ? convertToInput(survey.endAt) : new Date()
  );
  ...
  const handleSave = async () => {
    ...
    if (survey) {
      const questionsToEdit = questions.map((q) => ({
        text: q.text,
        surveyId,
      }));
      const ok = await dispatch(
        editSurvey({
          survey: { title, startAt, endAt, id: survey.id },
      ...
      const ok = await dispatch(
        createSurveys({ survey: { title, startAt, endAt }, questions })
      );
      if (ok) {
        navigate("/");
      }
  ...
  const onChange = (value, index) => {
    setQuestions(
      questions.map((q, i) =>
        index === i ? { text: value.currentTarget.value } : q
      )
    );
  };
    ...
    <Container>
      <div className={styles.container}>
        ...
        <div className={styles.dateWrapper}>
          <div className={styles.input}>
            ...
            <TextField
              ...
              type="date"
              value={startAt}
              onChange={(value) => setStartAt(value.currentTarget.value)}
            />
          </div>
          <div className={styles.input}>
            <label htmlFor="">Кінець опитування</label>
            <TextField
              min={convertToInput(startAt)}
              required
              type="date"
              value={endAt}
              onChange={(value) => setEndAt(value.currentTarget.value)}
            />
          </div>
        </div>
        ...
        <div className={styles.buttons}>
          <Button
            variant="contained"
            disabled={!questions.length || !title}
            onClick={handleSave}
          >
            Зберігти
          </Button>
